...
import matplotlib.pyplot as plt
import DeepLearningBatchGeneratorUtils.DataAugmentationGenerators as segDataAugm
from DeepLearningBatchGeneratorUtils import MultiThreadedGenerator
...
class BatchGen(DeepLearningBatchGeneratorUtils.DataGeneratorBase.BatchGeneratorBase):
    def generate_train_batch(self):
        ...
        return {"data":np.array(self._data[0][idx]), "seg": np.array(self._data[1][idx])}
...
def main():
    ...
    PATCH_SIZE = 512
    output_folder = "adam_BN_bottleneck/"
    ...
    net = build_UNet_relu_BN_bottleneck(n_input_channels=3, BATCH_SIZE=BATCH_SIZE, num_output_classes=2, pad='valid',
    ...
    train_generator = segDataAugm.center_crop_seg_generator(train_generator, net['output_segmentation'].output_shape[2])
    train_generator = MultiThreadedGenerator(train_generator, 6, 10)
    ...
    while epoch < N_EPOCHS:
        ...
        validation_generator = BatchGen((data_valid, target_valid), BATCH_SIZE, 25)
        validation_generator = DeepLearningBatchGeneratorUtils.DataAugmentationGenerators.random_crop_generator(validation_generator, PATCH_SIZE)
        validation_generator = DeepLearningBatchGeneratorUtils.DataAugmentationGenerators.center_crop_seg_generator(validation_generator, net['output_segmentation'].output_shape[2])
        validation_generator = MultiThreadedGenerator(validation_generator, 2, 4)
        for data_dict in validation_generator:
            target = data_dict["seg"]
            data = data_dict["data"]
        ...
        if (epoch != 0) and (np.mean(losses_val) > 1.5 * val_loss_old):
            ...
            test_gen = BatchGen((data_test, target_test), BATCH_SIZE, 20, 1)
            test_gen = DeepLearningBatchGeneratorUtils.DataAugmentationGenerators.center_crop_generator(test_gen, PATCH_SIZE)
            test_gen = DeepLearningBatchGeneratorUtils.DataAugmentationGenerators.center_crop_seg_generator(test_gen, net['output_segmentation'].output_shape[2])
